...
@Composable
fun MultiVideoMergeScreen(uiState: AppUiState, viewModel: MainViewModel) {
    ...
    LazyColumn(
        ...
        item {
            SectionCard(title = "输出 / 模式") {
                ...
                AppBody(
                    text = "快速合并要求所有视频编码参数完全相同；不同格式请选择强制转码。"
                )
            }
        }
        ...
        item {
            AppButton(
                ...
                onClick = { viewModel.runMultiVideoMerge(inputUris, output, outputName, outputContainer, mode) }
            ) {
